...
function M.init_values()
	M.startbme()
	gpio.config({ gpio = { GPIORESISTOR, GPIOVOLTEO_UP, GPIOVOLTEO_DOWN, GPIOHUMID }, dir = gpio.OUT })
    -- config inputs 
    gpio.config( { gpio={GPIOREEDS_DOWN,GPIOREEDS_UP}, dir=gpio.IN})
...
function M.rotation_change_dir()
	...
	if (upvalue == 1 and downvalue == 0) then
		M.rotate_up = false
	elseif (upvalue == 0 and downvalue == 1) then
		M.rotate_up = true
	else
